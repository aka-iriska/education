...
CalcDialog::CalcDialog(QWidget* parent)
{
    ...
    for (int i = 0; i < _btnDescr.size(); i++) {
        ...
        // если кнопка в основном блоке цифровых или "=" - разрешаем изменение всех размеров
        if (i >= GRID_KEYS + 3 || i < GRID_KEYS) // i >= GRID_KEYS + 3 - пропускаем upper block
    ...
    // связываем сигнал из m_pSignalMapper о нажатии со слотом clicked нашего класса
    connect(m_pSignalMapper, SIGNAL(mappedInt(int)), this, SLOT(clicked(int)));
...
void CalcDialog::calcPrevOp(int curOp)
{
    ...
    case DIV: {
        if (num != 0) m_Val /= num;
            ...
            QMessageBox::warning(this, "Предупреждение", "Вы не соблюли ОДЗ");
            op_correct = false;
        }
        break;
    }
    ...
    case LOG: {
        if (m_Val > 0 and num > 0 and num != 1) {
            ...
            QMessageBox::warning(this, "Предупреждение", "Вы не соблюли ОДЗ для log");
            op_correct = false;
        }
        break;
    }
