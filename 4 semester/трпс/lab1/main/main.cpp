...
int binarySearch(std::string name, Person* persons) {
    ...
    while (left <= right) {
        ...
        if (!persons[middle].delete_flag && persons[middle].name == name) {
            return middle;
        }
...
void defineDelete(int index, Person* persons) {
    if (index != -1) {
...
void deletePerson(Person* persons) {
    int write_index = 0;
    for (int read_index = 0; read_index < Person::count; read_index++) {
        if (!persons[read_index].delete_flag) {
            persons[write_index++] = persons[read_index];
        }
    }
    Person::count = write_index;
}
// Добавление элемента в массив
void addPerson(Person* persons, unsigned int groupCode, const std::string& name, const std::string& birthDate) {
    if (Person::count >= maxSize) {
        std::cerr << "Ошибка: Массив переполнен!" << std::endl;
        return;
    }

    persons[Person::count++] = Person(groupCode, name, birthDate);
}
int main()
{
    ...
    Person persons[maxSize] = {
        {2033, "Арбузов Иван Сергеевич", "15.04.2004"},
        {1015, "Дулина Ирина Арбузова", "01.10.1999"},
        {7001, "Абрикосов Владимир Владимирович", "31.12.1978"},
        {2033, "Иванова Арбуз Васильевна", "20.03.1970"},
        {3005, "Яблонева Карина Облепиховна", "31.02.2002"}
    };
    //Сортировка таблицы
    ...
    a = binarySearch("Иванова Надежда Васильевна", persons);
    defineDelete(a, persons);
    deletePerson(persons);
    std::cout << "Таблица с удалённым значением" << std::endl;
    printTable(persons);
    addPerson(persons, 1101, "Захаров Арбузер Владимирович", "12.12.1912");
    printTable(persons);
    return 0;
}
